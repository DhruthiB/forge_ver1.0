...
def generate_config(forge_path):
	...
	# make pids folder
	pid_path = os.path.join(forge_path, "config", "pids")
	...
	# make ACL files
	acl_rq_path = os.path.join(forge_path, "config", "redis_queue.acl")
	acl_redis_cache_path = os.path.join(forge_path, "config", "redis_cache.acl")
	open(acl_rq_path, "a").close()
	open(acl_redis_cache_path, "a").close()


def write_redis_config(template_name, context, forge_path):
	template = forge.config.env().get_template(template_name)

	if "config_path" not in context:
		context["config_path"] = os.path.abspath(os.path.join(forge_path, "config"))

	if "pid_path" not in context:
		context["pid_path"] = os.path.join(context["config_path"], "pids")

	with open(os.path.join(forge_path, "config", template_name), "w") as f:
