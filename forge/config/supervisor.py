...
from forge.utils import get_forge_name, which

logger = logging.getLogger(forge.PROJECT_NAME)


def generate_supervisor_config(forge_path, user=None, yes=False, skip_redis=False):
	"""Generate supervisor config for respective forge path"""
	if not user:
		user = getpass.getuser()

	config = Forge(forge_path).conf
	template = forge.config.env().get_template("supervisor.conf")
	...
	config = template.render(
		**{
			...
			"multi_queue_consumption": can_enable_multi_queue_consumption(forge_path),
			"supervisor_startretries": 10,
		}
	)

	conf_path = os.path.join(forge_path, "config", "supervisor.conf")
	...
	update_config({"restart_supervisor_on_update": True}, forge_path=forge_path)
	update_config({"restart_systemd_on_update": False}, forge_path=forge_path)
	sync_socketio_port(forge_path)
...
def sync_socketio_port(forge_path):
	# Backward compatbility: always keep redis_cache and redis_socketio port same
	common_config = get_config(forge_path=forge_path)
...
def can_enable_multi_queue_consumption(forge_path: str) -> bool:
	try:
		...
		stylo_version = Version(get_current_version("stylo", forge_path=forge_path))

		return stylo_version > supported_version
	except Exception:
		return False


def check_supervisord_config(user=None):
	"""From forge v5.x, we're moving to supervisor running as user"""
	# i don't think forge should be responsible for this but we're way past this now...
	# removed updating supervisord conf & reload in Aug 2022 - gavin@stylo.io
	import configparser
