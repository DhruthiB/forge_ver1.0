...
def setup_config(forge_path, additional_config=None):
	make_pid_folder(forge_path)
	forge_config = get_config(forge_path)
	forge_config.update(default_config)
	forge_config.update(get_gunicorn_workers())
	update_config_for_stylo(forge_config, forge_path)
	if additional_config:
		forge_config.update(additional_config)

	put_config(forge_config, forge_path)


def get_config(forge_path):
	return get_common_site_config(forge_path)


def get_common_site_config(forge_path):
	config_path = get_config_path(forge_path)
...
def put_config(config, forge_path="."):
	config_path = get_config_path(forge_path)
	with open(config_path, "w") as f:
		return json.dump(config, f, indent=1, sort_keys=True)


def update_config(new_config, forge_path="."):
	config = get_config(forge_path=forge_path)
	config.update(new_config)
	put_config(config, forge_path=forge_path)


def get_config_path(forge_path):
	return os.path.join(forge_path, "sites", "common_site_config.json")
...
def update_config_for_stylo(config, forge_path):
	ports = make_ports(forge_path)
...
def make_ports(forge_path):
	from urllib.parse import urlparse

	forgees_path = os.path.dirname(os.path.abspath(forge_path))
	...
	existing_ports = {}
	for folder in os.listdir(forgees_path):
		forge_path = os.path.join(forgees_path, folder)
...
def make_pid_folder(forge_path):
	pids_path = os.path.join(forge_path, "config", "pids")
