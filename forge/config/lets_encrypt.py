...
from forge.config.production_setup import service
from forge.config.site_config import get_domains, remove_domain, update_site_config
from forge.forge import Forge
from forge.utils import exec_cmd, which
from forge.utils.forge import update_common_site_config
from forge.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, forge_path, interactive):

	site_path = os.path.join(forge_path, "sites", site, "site_config.json")
	...
	if not Forge(forge_path).conf.get("dns_multitenant"):
		print("You cannot setup SSL without DNS Multitenancy")
		return

	create_config(site, custom_domain)
	run_certbot_and_setup_ssl(site, custom_domain, forge_path, interactive)
	setup_crontab()
...
def run_certbot_and_setup_ssl(site, custom_domain, forge_path, interactive=True):
	service("nginx", "stop")
	...
	if custom_domain:
		remove_domain(site, custom_domain, forge_path)
		domains = get_domains(site, forge_path)
		ssl_config["domain"] = custom_domain
		domains.append(ssl_config)
		update_site_config(site, {"domains": domains}, forge_path=forge_path)
	else:
		update_site_config(site, ssl_config, forge_path=forge_path)

	make_nginx_conf(forge_path)
	service("nginx", "start")
...
def setup_wildcard_ssl(domain, email, forge_path, exclude_base_domain):
