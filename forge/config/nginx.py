...
from forge.utils import get_forge_name


def make_nginx_conf(forge_path, yes=False, logging=None, log_format=None):
	conf_path = os.path.join(forge_path, "config", "nginx.conf")
	...
	if allow_rate_limiting:
		template_vars.update(
			{
				"forge_name_hash": hashlib.sha256(forge_name).hexdigest()[:16],
...
def make_forge_manager_nginx_conf(forge_path, yes=False, port=23624, domain=None):
	from forge.config.site_config import get_site_config

	template = forge.config.env().get_template("forge_manager_nginx.conf")
	forge_path = os.path.abspath(forge_path)
	sites_path = os.path.join(forge_path, "sites")

	config = Forge(forge_path).conf
	site_config = get_site_config(domain, forge_path=forge_path)
	forge_name = get_forge_name(forge_path)

	template_vars = {
		"port": port,
		"domain": domain,
		"forge_manager_site_name": "forge-manager.local",
		"sites_path": sites_path,
		"http_timeout": config.get("http_timeout"),
		"webserver_port": config.get("webserver_port"),
		"socketio_port": config.get("socketio_port"),
	...
	forge_manager_nginx_conf = template.render(**template_vars)

	conf_path = os.path.join(forge_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		click.confirm(
			"nginx.conf already exists and forge-manager configuration will be appended to it. Do you want to continue?",
			abort=True,
		)

	with open(conf_path, "a") as myfile:
		myfile.write(forge_manager_nginx_conf)


def prepare_sites(config, forge_path):
	sites = {
		"that_use_port": [],
		"that_use_dns": [],
		"that_use_ssl": [],
	...
	sites_configs = get_sites_with_config(forge_path=forge_path)
...
def get_sites_with_config(forge_path):
	from forge.forge import Forge
	from forge.config.site_config import get_site_config
	...
	for site in sites:
		try:
			site_config = get_site_config(site, forge_path=forge_path)
	...
	use_wildcard_certificate(forge_path, ret)

	return ret


def use_wildcard_certificate(forge_path, ret):
	"""
...
def get_error_pages():
	forge_app_path = os.path.abspath(forge.__path__[0])
	templates = os.path.join(forge_app_path, "config", "templates")
