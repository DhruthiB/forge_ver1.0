...
from forge.utils import exec_cmd, which, get_forge_name, get_cmd_output, log
from forge.utils.system import fix_prod_setup_perms
...
def setup_production_prerequisites():
	...
	if not which("fail2ban-client"):
		exec_cmd("forge setup role fail2ban")
	if not which("nginx"):
		exec_cmd("forge setup role nginx")
	if not which("supervisord"):
		exec_cmd("forge setup role supervisor")


def setup_production(user, forge_path=".", yes=False):
	print("Setting Up prerequisites...")
	setup_production_prerequisites()

	conf = Forge(forge_path).conf
	...
	if conf.get("restart_systemd_on_update"):
		...
		generate_systemd_config(forge_path=forge_path, user=user, yes=yes)
	else:
		print("Setting Up supervisor...")
		check_supervisord_config(user=user)
		generate_supervisor_config(forge_path=forge_path, user=user, yes=yes)

	print("Setting Up NGINX...")
	make_nginx_conf(forge_path=forge_path, yes=yes)
	fix_prod_setup_perms(forge_path, stylo_user=user)
	...
	if conf.get("restart_supervisor_on_update"):
		...
		supervisor_conf = os.path.join(
			get_supervisor_confdir(), f"{forge_name}.{supervisor_conf_extn}"
		)

		# Check if symlink exists, If not then create it.
		if not os.path.islink(supervisor_conf):
			os.symlink(
				os.path.abspath(os.path.join(forge_path, "config", "supervisor.conf")),
				supervisor_conf,
			)

	if not os.path.islink(nginx_conf):
		os.symlink(
			os.path.abspath(os.path.join(forge_path, "config", "nginx.conf")), nginx_conf
		)
...
def disable_production(forge_path="."):
	forge_name = get_forge_name(forge_path)
	conf = Forge(forge_path).conf
