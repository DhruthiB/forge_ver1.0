...
def print_forge_version(ctx, param, value):
...
def use_experimental_feature(ctx, param, value):
	...
	if value == "dynamic-feed":
		...
		forge.cli.dynamic_feed = True
		forge.cli.verbose = True
	else:
		from forge.exceptions import FeatureDoesNotExistError

		raise FeatureDoesNotExistError(f"Feature {value} does not exist")

	from forge.cli import is_envvar_warn_set

	if is_envvar_warn_set:
		return

	click.secho(
		"WARNING: forge is using it's new CLI rendering engine. This behaviour has"
