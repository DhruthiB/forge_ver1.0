...
import forge
from forge.exceptions import PatchError, ValidationError
...
def get_env_cmd(cmd: str, forge_path: str = ".") -> str:
	exact_location = os.path.abspath(
		os.path.join(forge_path, "env", "bin", cmd.strip("*"))
	)
	...
	existing_python_bins = glob(
		os.path.join(forge_path, "env", "**", "bin", cmd), recursive=True
	)
...
def update_node_packages(forge_path=".", apps=None, verbose=None):
	print("Updating node packages...")
	from distutils.version import LooseVersion

	from forge.utils.app import get_develop_version

	v = LooseVersion(get_develop_version("stylo", forge_path=forge_path))

	# After rollup was merged, stylo_version = 10.1
	# if develop_verion is 11 and up, only then install yarn
	if v < LooseVersion("11.x.x-develop"):
		update_npm_packages(forge_path, apps=apps, verbose=verbose)
	else:
		update_yarn_packages(forge_path, apps=apps, verbose=verbose)


def install_python_dev_dependencies(forge_path=".", apps=None, verbose=False):
	import forge.cli
	...
	if isinstance(apps, str):
		...
		apps = forge.get_installed_apps()

	for app in apps:
		pyproject_deps = None
		app_path = os.path.join(forge_path, "apps", app)
		...
		if os.path.exists(pyproject_path):
			...
			if pyproject_deps:
				if os.environ.get("BENCH_USE_UV"):
					forge.run(f"uv pip install {quiet_flag} --upgrade {pyproject_deps} --python {forge.python}")
				else:
					forge.run(f"{forge.python} -m pip install {quiet_flag} --upgrade {pyproject_deps}")

		if not pyproject_deps and os.path.exists(dev_requirements_path):
			if os.environ.get("BENCH_USE_UV"):
				forge.run(
					f"uv pip install {quiet_flag} --upgrade -r {dev_requirements_path} --python {forge.python}"
				)
			else:
				forge.run(
					f"{forge.python} -m pip install {quiet_flag} --upgrade -r {dev_requirements_path}"
				)
...
def _generate_dev_deps_pattern(pyproject_path):
	...
	with contextlib.suppress(KeyError):
		for pkg, version in pyproject_config["tool"]["forge"]["dev-dependencies"].items():
			op = "==" if "=" not in version else ""
			requirements_pattern += f"{pkg}{op}{version} "
	return requirements_pattern


def update_yarn_packages(forge_path=".", apps=None, verbose=None):
	...
	apps_dir = os.path.join(forge.name, "apps")
...
def update_npm_packages(forge_path=".", apps=None, verbose=None):
	verbose = forge.cli.verbose or verbose
	...
	with open(os.path.join(forge_path, "package.json"), "w") as f:
		f.write(json.dumps(package_json, indent=1, sort_keys=True))

	exec_cmd(npm_install, cwd=forge_path)
...
def validate_upgrade(from_ver, to_ver, forge_path="."):
	if to_ver >= 6 and not which("npm") and not which("node") and not which("nodejs"):
		raise Exception("Please install nodejs and npm")


def post_upgrade(from_ver, to_ver, forge_path="."):
	from forge.forge import Forge
	from forge.config import redis
	from forge.config.nginx import make_nginx_conf
	from forge.config.supervisor import generate_supervisor_config

	conf = Forge(forge_path).conf
	print("-" * 80 + f"Your forge was upgraded to version {to_ver}")

	if conf.get("restart_supervisor_on_update"):
		redis.generate_config(forge_path=forge_path)
		generate_supervisor_config(forge_path=forge_path)
		make_nginx_conf(forge_path=forge_path)
		print(
			"As you have setup your forge for production, you will have to reload"
...
def patch_sites(forge_path="."):
	from forge.forge import Forge
	from forge.utils.system import migrate_site

	forge = Forge(forge_path)

	for site in forge.sites:
		try:
			migrate_site(site, forge_path=forge_path)
		except subprocess.CalledProcessError:
			raise PatchError


def restart_supervisor_processes(forge_path=".", web_workers=False, _raise=False):
	...
	if cmd:
		forge.run(cmd, _raise=_raise)

	else:
		sudo = ""
		try:
			supervisor_status = get_cmd_output("supervisorctl status", cwd=forge_path)
		except subprocess.CalledProcessError as e:
			if e.returncode == 127:
				log("restart failed: Couldn't find supervisorctl in PATH", level=3)
				return
			sudo = "sudo "
			supervisor_status = get_cmd_output("sudo supervisorctl status", cwd=forge_path)
		...
		if web_workers and f"{forge_name}-web:" in supervisor_status:
			groups = [f"{forge_name}-web:\t"]

		elif f"{forge_name}-workers:" in supervisor_status:
			groups = [f"{forge_name}-web:", f"{forge_name}-workers:"]

		# backward compatibility
		elif f"{forge_name}-processes:" in supervisor_status:
			groups = [f"{forge_name}-processes:"]
		...
		for group in groups:
			failure = forge.run(f"{sudo}supervisorctl restart {group}", _raise=_raise)
			if failure:
				log(
					f"restarting supervisor group `{group}` failed. Use `forge restart` to retry.",
					level=3,
				)


def restart_systemd_processes(forge_path=".", web_workers=False, _raise=True):
	forge_name = get_forge_name(forge_path)
	exec_cmd(
		f"sudo systemctl stop -- $(systemctl show -p Requires {forge_name}.target | cut"
		" -d= -f2)",
		_raise=_raise,
	)
	exec_cmd(
		f"sudo systemctl start -- $(systemctl show -p Requires {forge_name}.target |"
		" cut -d= -f2)",
		_raise=_raise,
	)


def restart_process_manager(forge_path=".", web_workers=False):
	# only overmind has the restart feature, not sure other supported procmans do
	if which("overmind") and os.path.exists(os.path.join(forge_path, ".overmind.sock")):
		worker = "web" if web_workers else ""
		exec_cmd(f"overmind restart {worker}", cwd=forge_path)


def build_assets(forge_path=".", app=None, using_cached=False):
	command = "forge build"
	if app:
		command += f" --app {app}"
	...
	exec_cmd(command, cwd=forge_path, env=env)


def handle_version_upgrade(version_upgrade, forge_path, force, reset, conf):
	from forge.utils import log, pause_exec

	if version_upgrade[0]:
		if force:
			log(
				"""Force flag has been used for a major version change in Stylo and it's apps.
This will take significant time to migrate and might break custom apps.""",
				level=3,
			)
		else:
			print(
				f"""This update will cause a major version change in Stylo/ERPNext from {version_upgrade[1]} to {version_upgrade[2]}.
This would take significant time to migrate and might break custom apps."""
	...
	if not reset and conf.get("shallow_clone"):
		log(
			"""shallow_clone is set in your forge config.
However without passing the --reset flag, your repositories will be unshallowed.
To avoid this, cancel this operation and run `forge update --reset`.
...
To avoid seeing this warning, set shallow_clone to false in your common_site_config.json
	...
	if version_upgrade[0] or (not version_upgrade[0] and force):
		validate_upgrade(version_upgrade[1], version_upgrade[2], forge_path=forge_path)
...
def update(
	...
	from forge import patches
	from forge.app import pull_apps
	from forge.forge import Forge
	from forge.config.common_site_config import update_config
	from forge.exceptions import CannotUpdateReleaseForge
	from forge.utils.app import is_version_upgrade
	from forge.utils.system import backup_all_sites

	forge_path = os.path.abspath(".")
	forge = Forge(forge_path)
	patches.run(forge_path=forge_path)
	conf = forge.conf

	if conf.get("release_forge"):
		raise CannotUpdateReleaseForge("Release forge detected, cannot update!")
	...
	handle_version_upgrade(version_upgrade, forge_path, force, reset, conf)

	conf.update({"maintenance_mode": 1, "pause_scheduler": 1})
	update_config(conf, forge_path=forge_path)

	if backup:
		print("Backing up sites...")
		backup_all_sites(forge_path=forge_path)

	if pull:
		print("Updating apps source...")
		pull_apps(apps=apps, forge_path=forge_path, reset=reset)

	if requirements:
		print("Setting up requirements...")
		forge.setup.requirements()

	if patch:
		print("Patching sites...")
		patch_sites(forge_path=forge_path)
	...
	if version_upgrade[0] or (not version_upgrade[0] and force):
		post_upgrade(version_upgrade[1], version_upgrade[2], forge_path=forge_path)

	forge.reload(web=False, supervisor=restart_supervisor, systemd=restart_systemd)

	conf.update({"maintenance_mode": 0, "pause_scheduler": 0})
	update_config(conf, forge_path=forge_path)

	print(
		"_" * 80 + "\nForge: Deployment tool for Stylo and Stylo Applications"
		" (https://stylo.io/forge).\nOpen source depends on your contributions, so do"
		" give back by submitting bug reports, patches and fixes and be a part of the"
		" community :)"
	)


def clone_apps_from(forge_path, clone_from, update_app=True):
	from forge.app import install_app

	print(f"Copying apps from {clone_from}...")
	subprocess.check_output(["cp", "-R", os.path.join(clone_from, "apps"), forge_path])

	node_modules_path = os.path.join(clone_from, "node_modules")
	if os.path.exists(node_modules_path):
		print(f"Copying node_modules from {clone_from}...")
		subprocess.check_output(["cp", "-R", node_modules_path, forge_path])
	...
	def setup_app(app):
		...
		install_app(app, forge_path, restart_forge=False)
...
def remove_backups_crontab(forge_path="."):
	...
	user = Forge(forge_dir).conf.get("stylo_user")
...
def set_mariadb_host(host, forge_path="."):
	update_common_site_config({"db_host": host}, forge_path=forge_path)


def set_redis_cache_host(host, forge_path="."):
	if isinstance(host, str) and not host.lower().startswith("redis"):
		host = f"redis://{host}"

	update_common_site_config({"redis_cache": host}, forge_path=forge_path)


def set_redis_queue_host(host, forge_path="."):
	if isinstance(host, str) and not host.lower().startswith("redis"):
		host = f"redis://{host}"

	update_common_site_config({"redis_queue": host}, forge_path=forge_path)


def set_redis_socketio_host(host, forge_path="."):
	if isinstance(host, str) and not host.lower().startswith("redis"):
		host = f"redis://{host}"

	update_common_site_config({"redis_socketio": host}, forge_path=forge_path)


def update_common_site_config(ddict, forge_path="."):
	filename = os.path.join(forge_path, "sites", "common_site_config.json")
...
def validate_app_installed_on_sites(app, forge_path="."):
	print("Checking if app installed on active sites...")
	ret = check_app_installed(app, forge_path=forge_path)

	if ret is None:
		check_app_installed_legacy(app, forge_path=forge_path)
	else:
		return ret


def check_app_installed(app, forge_path="."):
	try:
		out = subprocess.check_output(
			["forge", "--site", "all", "list-apps", "--format", "json"],
...
def check_app_installed_legacy(app, forge_path="."):
	site_path = os.path.join(forge_path, "sites")

	for site in os.listdir(site_path):
		req_file = os.path.join(site_path, site, "site_config.json")
		if os.path.exists(req_file):
			out = subprocess.check_output(
				["forge", "--site", site, "list-apps"], cwd=forge_path
			).decode("utf-8")
...
def validate_branch():
	from forge.forge import Forge
	from forge.utils.app import get_current_branch

	apps = Forge(".").apps

	installed_apps = set(apps)
	check_apps = {"stylo", "erpnext"}
...
To switch to your required branch, run the following commands: forge switch-to-branch [branch-name]"""
			)
...
def cache_list() -> None:
	...
	for item in get_forge_cache_path("apps").iterdir():
