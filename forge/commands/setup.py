...
from forge.utils import exec_cmd, run_playbook, which
from forge.utils.cli import SugaredOption


@click.group(help="Setup command group for enabling setting up a Stylo environment")
def setup():
	pass
...
def setup_sudoers(user):
	from forge.utils.system import setup_sudoers
...
def setup_nginx(yes=False, logging="combined", log_format=None):
	...
	make_nginx_conf(forge_path=".", yes=yes, logging=logging, log_format=log_format)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
	from forge.config.production_setup import reload_nginx
...
def setup_supervisor(user=None, yes=False, skip_redis=False, skip_supervisord=False):
	from forge.utils import get_cmd_output
	from forge.config.supervisor import (
	...
	generate_supervisor_config(forge_path=".", user=user, yes=yes, skip_redis=skip_redis)
...
@click.command("fonts", help="Add Stylo fonts to system")
def setup_fonts():
	from forge.utils.system import setup_fonts

	setup_fonts()


@click.command(
	"production", help="Setup Stylo production environment for specific user"
)
@click.argument("user")
@click.option("--yes", help="Yes to regeneration config", is_flag=True, default=False)
def setup_production(user, yes=False):
	from forge.config.production_setup import setup_production

	setup_production(user=user, yes=yes)


@click.command("backups", help="Add cronjob for forge backups")
def setup_backups():
...
@click.command("env", help="Setup Python environment for forge")
@click.option(
...
def setup_env(python="python3"):
	...
	return Forge(".").setup.env(python=python)
...
def setup_firewall(ssh_port=None, force=False):
	...
	run_playbook("roles/forge/tasks/setup_firewall.yml", {"ssh_port": ssh_port})
...
def set_ssh_port(port, force=False):
	...
	run_playbook("roles/forge/tasks/change_ssh_port.yml", {"ssh_port": port})
...
def setup_letsencrypt(site, custom_domain, non_interactive):
	from forge.config.lets_encrypt import setup_letsencrypt

	setup_letsencrypt(site, custom_domain, forge_path=".", interactive=not non_interactive)


@click.command(
	"wildcard-ssl", help="Setup wildcard SSL certificate for multi-tenant forge"
)
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	from forge.config.lets_encrypt import setup_wildcard_ssl

	setup_wildcard_ssl(
		domain, email, forge_path=".", exclude_base_domain=exclude_base_domain
	)


@click.command("procfile", help="Generate Procfile for forge start")
def setup_procfile():
...
def setup_requirements(node=False, python=False, dev=False, apps=None):
	...
	if not (node or python or dev):
		forge.setup.requirements(apps=apps)

	elif not node and not dev:
		forge.setup.python(apps=apps)

	elif not python and not dev:
		forge.setup.node(apps=apps)

	else:
		from forge.utils.forge import install_python_dev_dependencies
...
@click.command(
	"manager",
	help="Setup forge-manager.local site with the forge_manager app installed on it",
)
@click.option(
	"--yes", help="Yes to regeneration of nginx config file", default=False, is_flag=True
)
@click.option(
	"--port", help="Port on which you want to run forge manager", default=23624
)
@click.option("--domain", help="Domain on which you want to run forge manager")
def setup_manager(yes=False, port=23624, domain=None):
	from forge.forge import Forge
	from forge.config.nginx import make_forge_manager_nginx_conf

	create_new_site = True

	if "forge-manager.local" in os.listdir("sites"):
		create_new_site = click.confirm("Site already exists. Overwrite existing site?")

	if create_new_site:
		exec_cmd("forge new-site --force forge-manager.local")

	if "forge_manager" in os.listdir("apps"):
		print("App already exists. Skipping app download.")
	else:
		exec_cmd("forge get-app forge_manager")

	exec_cmd("forge --site forge-manager.local install-app forge_manager")

	forge_path = "."
	forge = Forge(forge_path)

	if forge.conf.get("restart_supervisor_on_update") or forge.conf.get(
		"restart_systemd_on_update"
	):
		# implicates a production setup or so I presume
		if not domain:
			print(
				"Please specify the site name on which you want to host forge-manager using the 'domain' flag"
			)
			sys.exit(1)

		if domain not in forge.sites:
			raise Exception("No such site")

		make_forge_manager_nginx_conf(forge_path, yes=yes, port=port, domain=domain)
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
	...
	from forge.config.site_config import add_domain

	add_domain(site, domain, ssl_certificate, ssl_certificate_key, forge_path=".")
...
def remove_domain(domain, site=None):
	...
	from forge.config.site_config import remove_domain

	remove_domain(site, domain, forge_path=".")
...
def sync_domains(domain=None, site=None):
	...
	from forge.config.site_config import sync_domains

	changed = sync_domains(site, domains, forge_path=".")
...
@click.option("--stop", help="Stop forge services", is_flag=True, default=False)
...
def setup_systemd(
	...
	from forge.config.systemd import generate_systemd_config
