...
from forge.utils import set_git_remote_url
from forge.utils.app import get_remote
...
@click.command('remote-reset-url', help="Reset app remote url to stylo official")
@click.argument('app')
def remote_reset_url(app):
	git_url = f"https://github.com/stylo/{app}.git"
	set_git_remote_url(git_url)


@click.command('remote-urls', help="Show apps remote url")
def remote_urls():
	for app in Forge(".").apps:
