...
from forge.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help="Change forge configuration")
def config():
	pass
...
@click.command(
	"dns_multitenant", help="Enable/Disable forge multitenancy on running forge update"
)
@click.argument("state", type=click.Choice(["on", "off"]))
...
def set_common_config(configs):
	...
	update_config(common_site_config, forge_path=".")


@click.command(
	"remove-common-config", help="Remove specific keys from current forge's common config"
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
	from forge.forge import Forge

	common_site_config = Forge(".").conf
	for key in keys:
