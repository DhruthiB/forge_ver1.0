...
from forge.app import pull_apps
from forge.utils.forge import post_upgrade, patch_sites, build_assets


@click.command(
	"update",
	help="Performs an update operation on current forge. Without any flags will backup, pull, setup requirements, build, run patches and restart forge. Using specific flags will only do certain tasks instead of all",
)
@click.option("--pull", is_flag=True, help="Pull updates for all the apps in forge")
@click.option("--apps", type=str)
@click.option("--patch", is_flag=True, help="Run migrations for all sites in the forge")
@click.option("--build", is_flag=True, help="Build JS and CSS assets for the forge")
@click.option(
	"--requirements",
	is_flag=True,
	help="Update requirements. If run alone, equivalent to `forge setup requirements`",
)
@click.option(
...
def update(
	...
	from forge.utils.forge import update
...
def switch_to_branch(branch, apps, upgrade=False):
	from forge.utils.app import switch_to_branch
...
def switch_to_develop(upgrade=False):
	"Switch stylo and erpnext to develop branch"
	from forge.utils.app import switch_to_develop

	switch_to_develop(apps=["stylo", "erpnext"])
