...
from forge.utils.forge import build_assets, install_python_dev_dependencies
from forge.utils.render import step
...
class AppMeta:
	def __init__(self, name: str, branch: str | None = None, to_clone: bool = True):
		...
		name (str): This could look something like
				1. https://github.com/stylo/healthcare.git
				2. git@github.com:stylo/healthcare.git
				3. stylo/healthcare@develop
				4. healthcare
		...
		class Healthcare(AppConfig):
				dependencies = [{"stylo/erpnext": "~13.17.0"}]
		"""
	...
	def setup_details(self):
		...
		if self.forge and os.path.exists(os.path.join(self.forge.name, "apps", self.name)):
			self.mount_path = os.path.join(self.forge.name, "apps", self.name)
			self.from_apps = True
...
class App(AppMeta):
	...
	def get(self):
		...
		shallow = "--depth 1" if self.forge.shallow_clone else ""
		...
		self.forge.run(
			f"{cmd} {args}",
			cwd=os.path.join(self.forge.name, "apps"),
		)
	...
	def install(
		...
		import forge.cli
		from forge.utils.app import get_app_name

		self.validate_app_dependencies()

		verbose = forge.cli.verbose or verbose
		app_name = get_app_name(self.forge.name, self.app_name)
		if not resolved and self.app_name != "stylo" and not ignore_resolution:
			click.secho(
		...
		install_app(
			app=app_name,
			tag=self.tag,
			forge_path=self.forge.name,
			verbose=verbose,
			skip_assets=skip_assets,
			restart_forge=restart_forge,
	...
	def uninstall(self):
		if os.environ.get("BENCH_USE_UV"):
			self.forge.run(f"uv pip uninstall {self.name} --python {self.forge.python}")
		else:
			self.forge.run(f"{self.forge.python} -m pip uninstall -y {self.name}")

	def _get_dependencies(self):
		from forge.utils.app import get_required_deps, required_apps_from_hooks
	...
	def get_pyproject(self) -> Optional[dict]:
		from forge.utils.app import get_pyproject

		if self.pyproject:
			return self.pyproject

		apps_path = os.path.join(os.path.abspath(self.forge.name), "apps")
	...
	def validate_app_dependencies(self, throw=False) -> None:
		...
		deps: Optional[dict] = (
			pyproject.get("tool", {}).get("forge", {}).get("stylo-dependencies")
		)
		if not deps:
			return
	...
	forge sub directories. If we assume deterministic builds
	...
	def get_app_path(self) -> Path:
		return Path(self.forge.name) / "apps" / self.app_name

	def get_app_cache_temp_path(self, is_compressed=False) -> Path:
		cache_path = get_forge_cache_path("apps")
	...
	def get_cached(self) -> bool:
		...
		click.secho(
			f"Forge app-cache: extracting {self.app_name} from {cache_path.as_posix()}",
		)
		...
		with tarfile.open(cache_path, mode) as tar:
			...
			try:
				...
				click.secho(
					f"Forge app-cache: extraction succeeded for {self.app_name}",
					fg="green",
				)
			except Exception:
				message = f"Forge app-cache: extraction failed for {self.app_name}"
				click.secho(
					message,
					fg="yellow",
				)
	...
	def set_cache(self, compress_artifacts=False) -> bool:
		...
		message = f"Forge app-cache: caching {self.app_name}"
		...
		try:
			...
			click.secho(
				f"Forge app-cache: caching succeeded for {self.app_name} as {hashed_path.as_posix()}",
				fg="green",
			)

			success = True
		except Exception as exc:
			log(f"Forge app-cache: caching failed for {self.app_name} {exc}", level=3)
			success = False
		finally:
			os.chdir(cwd)
		return success
...
def can_stylo_use_cached(app: App) -> bool:
	min_stylo = get_required_stylo_version(app)
	if not min_stylo:
		return False

	try:
		return sv.Version(min_stylo) in sv.SimpleSpec(">=15.12.0")
	except ValueError:
	...
	try:
		...
		"""
		return sv.Version("15.12.0") not in sv.SimpleSpec(min_stylo)
	except ValueError:
		click.secho(
			f"Forge app-cache: invalid value found for stylo version '{min_stylo}'",
			fg="yellow",
		)
		# Invalid expression
		return False
...
def validate_dependency(app: App, dep: str, req_version: str, throw=False) -> None:
	dep_path = Path(app.forge.name) / "apps" / dep
	if not dep_path.is_dir():
		click.secho(f"Required stylo-dependency '{dep}' not found.", fg="yellow")
		if throw:
			sys.exit(1)
		return
	...
	if sv.Version(dep_version) not in sv.SimpleSpec(req_version):
		click.secho(
			f"Installed stylo-dependency '{dep}' version '{dep_version}' "
...
def get_required_stylo_version(app: App) -> Optional[str]:
	pyproject = app.get_pyproject() or {}

	# Reference: https://github.com/stylo/forge/issues/1524
	req_stylo = (
		pyproject.get("tool", {})
		.get("forge", {})
		.get("stylo-dependencies", {})
		.get("stylo")
	)

	if not req_stylo:
		click.secho(
			"Required stylo version not set in pyproject.toml, "
			"please refer: https://github.com/stylo/forge/issues/1524",
			fg="yellow",
		)
...
def remove_unused_node_modules(app_path: Path) -> None:
	...
	Since most popular Stylo apps use Vite to build their frontends,
	...
	for p in app_path.iterdir():
		...
		if can_delete:
			click.secho(
				f"Forge app-cache: removing {node_modules.as_posix()}",
				fg="yellow",
			)
			shutil.rmtree(node_modules)


def make_resolution_plan(app: App, forge: "Forge"):
	"""
	...
	for app_name in app._get_dependencies():
		dep_app = App(app_name, forge=forge)
		is_valid_stylo_branch(dep_app.url, dep_app.branch)
		...
		resolution.update(make_resolution_plan(dep_app, forge))
		app.local_resolution = [repo_name for repo_name, _ in reversed(resolution.items())]
	return resolution


def get_excluded_apps(forge_path="."):
	try:
		with open(os.path.join(forge_path, "sites", "excluded_apps.txt")) as f:
			return f.read().strip().split("\n")
	except OSError:
		return []


def add_to_excluded_apps_txt(app, forge_path="."):
	if app == "stylo":
		raise ValueError("Stylo app cannot be excluded from update")
	if app not in os.listdir("apps"):
		raise ValueError(f"The app {app} does not exist")
	apps = get_excluded_apps(forge_path=forge_path)
	if app not in apps:
		apps.append(app)
		return write_excluded_apps_txt(apps, forge_path=forge_path)


def write_excluded_apps_txt(apps, forge_path="."):
	with open(os.path.join(forge_path, "sites", "excluded_apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, forge_path="."):
	apps = get_excluded_apps(forge_path=forge_path)
	if app in apps:
		apps.remove(app)
		return write_excluded_apps_txt(apps, forge_path=forge_path)
...
def get_app(
	...
	"""forge get-app clones a Stylo App from remote (GitHub or any other git server),
	and installs it on the current forge. This also resolves dependencies based on the
	apps' required_apps defined in the hooks.py file.

	If the forge_path is not a forge directory, a new forge is created named using the
	git_url parameter.
	"""
	import forge as _forge
	import forge.cli as forge_cli
	from forge.forge import Forge
	from forge.utils.app import check_existing_dir

	use_cached_app = False

	forge = Forge(forge_path)
	app = App(
		git_url, branch=branch, forge=forge, soft_link=soft_link, cache_key=cache_key
	)
	...
	if urlparse(git_url).scheme == "file" and cache_key:
		if use_cached_app := app.get_cached():
			...
			app = App(git_url, branch=branch, forge=forge, soft_link=soft_link, cache_key=cache_key)

	git_url = app.url
	repo_name = app.repo
	branch = app.tag
	forge_setup = False
	restart_forge = not init_forge
	stylo_path, stylo_branch = None, None

	if resolve_deps:
		resolution = make_resolution_plan(app, forge)
		...
		if "stylo" in resolution:
			# Todo: Make stylo a terminal dependency for all stylo apps.
			stylo_path, stylo_branch = resolution["stylo"].url, resolution["stylo"].tag

	if not is_forge_directory(forge_path):
		if not init_forge:
			click.secho(
				f"{os.path.realpath(forge_path)} is not a valid forge directory. "
				"Run with --init-forge if you'd like to create a Forge too.",
				fg="red",
			)
			sys.exit(1)
		...
		forge_path = get_available_folder_name(f"{app.repo}-forge", forge_path)
		init(
			path=forge_path,
			stylo_path=stylo_path,
			stylo_branch=stylo_branch or branch,
		)
		os.chdir(forge_path)
		forge_setup = True

	if forge_setup and forge_cli.from_command_line and forge_cli.dynamic_feed:
		_forge.LOG_BUFFER.append(
			{
	...
	dir_already_exists, cloned_path = check_existing_dir(forge_path, repo_name)
	...
	if (
		...
		app.install(verbose=verbose, skip_assets=skip_assets, restart_forge=restart_forge)
...
def install_resolved_deps(
	...
	if "stylo" in resolution:
		# Terminal dependency
		del resolution["stylo"]

	for repo_name, app in reversed(resolution.items()):
		existing_dir, path_to_app = check_existing_dir(forge_path, repo_name)
		if existing_dir:
			is_compatible = False

			try:
				installed_branch = forge.apps.states[repo_name]["resolution"]["branch"].strip()
			except Exception:
				installed_branch = (
...
def new_app(app, no_git=None, forge_path="."):
	if forge.FRAPPE_VERSION in (0, None):
		click.secho(
			f"{os.path.realpath(forge_path)} is not a valid forge directory.",
			fg="red",
		)
	...
	apps = os.path.abspath(os.path.join(forge_path, "apps"))
	args = ["make-app", apps, app]
	if no_git:
		if forge.FRAPPE_VERSION < 14:
			click.secho("Stylo v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_stylo_cmd(*args, forge_path=forge_path)
	install_app(app, forge_path=forge_path)
...
def install_app(
	...
	verbose = forge_cli.verbose or verbose
	quiet_flag = "" if verbose else "--quiet"
	cache_flag = "--no-cache-dir" if no_cache else ""

	app_path = os.path.realpath(os.path.join(forge_path, "apps", app))
	...
	if app == "stylo" and get_current_stylo_version(forge_path) >= 16:
		check_pkg_config()
	...
	if os.environ.get("BENCH_USE_UV"):
		try:
			forge.run(
				f"uv pip install {quiet_flag} --upgrade -e {app_path} {cache_flag} --python {forge.python}",
				env=env,
			)
		...
		try:
			forge.run(
				f"{forge.python} -m pip install {quiet_flag} --upgrade -e {app_path} {cache_flag}",
				env=env,
			)
		except Exception as e:
			print(f"Error occured during app install: {e}", file=sys.stderr)
	...
	if conf.get("developer_mode"):
		install_python_dev_dependencies(
			apps=app, forge_path=forge_path, verbose=verbose
		)
	...
	if not using_cached and os.path.exists(os.path.join(app_path, "package.json")):
		...
		forge.run(yarn_install, cwd=app_path)

	forge.apps.sync(app_name=app, required=resolution, branch=tag, app_dir=app_path)

	if not skip_assets:
		build_assets(forge_path=forge_path, app=app, using_cached=using_cached)

	if restart_forge:
		# Avoiding exceptions here as production might not be set-up
		# OR we might just be generating docker images.
		forge.reload(_raise=False)


def pull_apps(apps=None, forge_path=".", reset=False):
	"""Check all apps if there no local changes, pull"""
	from forge.forge import Forge
	from forge.utils.app import get_current_branch, get_remote

	forge = Forge(forge_path)
	rebase = "--rebase" if forge.conf.get("rebase_on_pull") else ""
	apps = apps or forge.apps
	excluded_apps = forge.excluded_apps
	...
	if not reset:
		for app in apps:
			if app in excluded_apps:
				...
				continue
			app_dir = get_repo_dir(app, forge_path=forge_path)
...
2. Temporarily remove your changes with "git stash" or discard them completely
	with "forge update --reset" or for individual repositries "git reset --hard"
	...
	for app in apps:
		...
		if os.path.exists(os.path.join(app_dir, ".git")):
			...
			if not remote:
				...
				add_to_excluded_apps_txt(app, forge_path=forge_path)
				print(
			...
			if not forge.conf.get("shallow_clone") or not reset:
				...
				if is_shallow:
					...
					forge.run(f"git fetch {remote} --unshallow", cwd=app_dir)

			branch = get_current_branch(app, forge_path=forge_path)
			logger.log(f"pulling {app}")
			if reset:
				reset_cmd = f"git reset --hard {remote}/{branch}"
				if forge.conf.get("shallow_clone"):
					forge.run(f"git fetch --depth=1 --no-tags {remote} {branch}", cwd=app_dir)
					forge.run(reset_cmd, cwd=app_dir)
					forge.run("git reflog expire --all", cwd=app_dir)
					forge.run("git gc --prune=all", cwd=app_dir)
				else:
					forge.run("git fetch --all", cwd=app_dir)
					forge.run(reset_cmd, cwd=app_dir)
			else:
				forge.run(f"git pull {rebase} {remote} {branch}", cwd=app_dir)
			forge.run('find . -name "*.pyc" -delete', cwd=app_dir)


def use_rq(forge_path):
	forge_path = os.path.abspath(forge_path)
	celery_app = os.path.join(forge_path, "apps", "stylo", "stylo", "celery_app.py")
	return not os.path.exists(celery_app)


def get_repo_dir(app, forge_path="."):
	return os.path.join(forge_path, "apps", app)
...
def install_apps_from_path(path, forge_path="."):
...
def validate_cache_and_get_path(app_name: str, cache_key: str) -> "Optional[Path]":
	...
	if not cache_path.is_file():
		click.secho(
			f"Forge app-cache: file check failed for {cache_path.as_posix()}, skipping cache",
			fg="yellow",
		)
	...
	if not is_cache_hash_valid(cache_path):
		click.secho(
			f"Forge app-cache: hash validation failed for {cache_path.as_posix()}, skipping cache",
			fg="yellow",
		)
		unlink_no_throw(cache_path)
		return
